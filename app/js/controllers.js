var bookStoreCtrls = angular.module('bookStoreCtrls', ["xeditable"]);

bookStoreCtrls.controller('ExampleCtrl', function($scope, $http, $timeout) {
        ...
        $http.get("js/xeditable-groups.json")
...
bookStoreCtrls.controller('ExampleCtrl',
 function($scope, $filter, $http, editableOptions, editableThemes, $q,$timeout) {
 	...
 	$scope.names = ["Emil", "Tobias", "Linus"];
    	...
    	$scope.myHeader = "Hello World!";
        $http.get("../server/xeditable-groups.json")
	    	.success(function(response) {$scope.response = response});
...
bookStoreCtrls.controller('appListCtrl', ['$scope',
    function($scope) {
        $scope.name = "HELLO";
    }
]);
